...
struct APIConnectionView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            HStack {
                Label("Sunburst API 연결", systemImage: "sun.max").font(StudioTypography.title)
                Spacer()
                Button { dismiss() } label: { Image(systemName: "xmark") }.buttonStyle(.plain).accessibilityLabel("API 연결 닫기")
            }
            Text("ChatGPT 구독과 별도로 사용하는 OpenAI API입니다. 생성 비용은 연결한 API 계정에 청구됩니다.")
                .font(StudioTypography.body).fixedSize(horizontal: false, vertical: true)
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                HStack { Link("계정 및 결제 열기 ↗", destination: URL(string: "https://platform.openai.com/settings/organization/billing/overview")!); Link("요금표 ↗", destination: URL(string: "https://developers.openai.com/api/docs/models/gpt-image-2.5-sunburst")!); Link("사용량 ↗", destination: URL(string: "https://platform.openai.com/usage")!) }
                ...
                step("3", title: "키를 붙여 넣고 연결 확인", text: "Sunburst 모델 조회로 접근을 확인합니다. 이 단계에서는 이미지를 생성하지 않습니다.")
                ...
                Toggle("API 사용 요금이 별도로 청구됨을 확인했습니다", isOn: $acceptsBilling).font(StudioTypography.supporting)
