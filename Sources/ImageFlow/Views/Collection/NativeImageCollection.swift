...
final class ImageCollectionItem: NSCollectionViewItem {
    ...
    func render(asset: Asset, store: WorkspaceStore, selected: Bool, edit: @escaping () -> Void, attach: @escaping () -> Void) {
        ...
        view.setAccessibilityLabel("\(asset.title), \(asset.width) × \(asset.height)\(asset.isFavorite ? ", 후보" : "")")
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(alignment: .center, spacing: 6) {
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text("\(asset.width) × \(asset.height)").font(.system(size: 11)).foregroundStyle(.secondary).lineLimit(1)
                }
                Spacer(minLength: 0)
                if hovered || selected {
