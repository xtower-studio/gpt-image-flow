            ...
            ZStack(alignment: .topLeading) {
                ...
                if nodes.isEmpty {
                    VStack(spacing: 10) {
                        ...
                        Text("아이디어를 자유롭게 펼쳐 보세요").font(StudioTypography.section)
                        Text("오른쪽 참조 영역에 이미지를 놓고 첫 시안을 생성하세요.").font(StudioTypography.body).foregroundStyle(.secondary)
                ...
                HStack(spacing: 12) {
                    ...
                    Text("\(Int(viewport.scale*100))%").monospacedDigit().font(StudioTypography.metadata).frame(width: 40)
    ...
    func nodeView(_ node: Node) -> some View {
        VStack(spacing: 0) {
            HStack {
                ...
                Text(node.asset?.isReference == true ? "참조 이미지" : node.job != nil ? "생성 작업" : "이미지").font(StudioTypography.metadata)
                Spacer()
                if let asset = node.asset, !asset.isReference, let model = asset.actualModelLabel {
                    Text(model).font(StudioTypography.metadata).foregroundStyle(.secondary)
                } else if let job = node.job {
                    Text("\(job.requestModeLabel) · \(job.expectedImageCount)장").font(StudioTypography.metadata).foregroundStyle(.secondary)
                }
            ...
            if let asset = node.asset {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    Label(job.state.label, systemImage: job.state.isRunning ? "sparkles" : job.state == .responded ? "text.bubble" : job.state.needsAttention ? "exclamationmark.triangle" : "clock")
                        .font(StudioTypography.section).foregroundStyle(job.state == .failed || job.state == .needsReview ? Color.orange : Color.primary)
                    if job.state.isRunning { ProgressView().controlSize(.small) }
                    Text(job.responseText ?? job.error ?? job.prompt).font(StudioTypography.body).lineLimit(8).textSelection(.enabled)
                    Spacer(minLength: 0)
                    if job.state.needsAttention { Button(job.state == .responded ? "답변 보기 · 이어서 요청" : "상태 보기") { openJob(job) }.buttonStyle(.bordered) }
                    else { Text(job.label).font(StudioTypography.metadata).foregroundStyle(.secondary).lineLimit(1) }
