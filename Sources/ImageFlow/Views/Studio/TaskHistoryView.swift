...
struct TaskHistoryView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            }.padding(.horizontal, PanelSpacing.inset).padding(.top, PanelSpacing.top).padding(.bottom, 16)
            if store.journal.paused {
                Label(store.journal.pauseReason ?? "대기열이 일시정지되었습니다", systemImage: "pause.circle").font(StudioTypography.supporting).foregroundStyle(.orange).padding(.horizontal, PanelSpacing.inset).padding(.bottom, 12)
            }
                    ...
                    LazyVStack(spacing: 8) {
                        ForEach(jobs) { job in
                            Button { selectedJobID = job.id } label: {
                                ...
                                }.contentShape(Rectangle()).padding(PanelSpacing.card).panelSurface(radius: 16)
                            }.buttonStyle(.plain)
                        }
                    }.padding(.horizontal, PanelSpacing.inset).padding(.bottom, 16)
                }.panelScrollEdges()
            }
        }
    }
