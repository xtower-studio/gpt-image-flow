    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                ...
                else { Text("작업 기록").font(.headline) }
                Spacer()
                Button(action: engine.pauseOrResume) { Image(systemName: store.journal.paused ? "play" : "pause") }.help(store.journal.paused ? "대기열 계속" : "대기열 일시정지").buttonStyle(.borderless)
            }.padding(16)
            ...
            if let selectedJobID { ScrollView { JobResponseView(jobID: selectedJobID).id(selectedJobID) } }
                        ...
                        ForEach(jobs) { job in
                            Button { selectedJobID = job.id } label: {
                                HStack(alignment: .top, spacing: 12) {
                                    if let asset = job.results.first { AssetThumbnail(url: store.vault.thumbnail(asset), fit: false).frame(width: 46, height: 46).clipped().clipShape(RoundedRectangle(cornerRadius: 5)) }
